...
// Este CLI enseña el plan y pregunta antes de ejecutarlo, así que no puede
// dejar que `start()` lo apruebe por dentro. Con `approvalMode: 'auto'` por
// defecto lo hacía: imprimía "PLAN PROPUESTO" con los pasos ya marcados [x] y
// preguntaba "¿Aprobar el plan?" cuando los archivos ya estaban escritos.
// Con --auto se salta la pregunta, que es justo para lo que existe esa opción.
engine.config.set('agent.autoApprovePlan', !!args.auto);
engine.config.set('agent.autoRunSteps', !!args.auto);
...
// Con --auto, `start()` ya lo aprobó y lo ejecutó por dentro.
if (!args.auto) {
