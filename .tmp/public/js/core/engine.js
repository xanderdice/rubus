...
import { Config, approvalPolicy } from './config.js';
...
export class Engine {
    constructor({ platform, bus }) {
        ...
        this.memory = new ProjectMemory({ platform, config: this.config, logger: this.logger });
        // The verifier reads the repo map's conventions to find the project's
        // own test command when the user has not configured one.
        this.verifier = new Verifier({
            platform, config: this.config, security: this.security,
            logger: this.logger, repoMap: this.repoMap
        });
        ...
        /** ¿Se ejecutó algún comando de consola en esta tarea? */
    ...
    async setWorkspace(root, { persist = true } = {}) {
        ...
        this.memory.loadedRoot = '';
        // Otro proyecto, otro `npm test`: ni la medida ni el permiso valen aquí.
        this.verifier.reset();
    ...
    async start(task) {
        ...
        // La línea base es de ESTA tarea. Una suite que estaba roja en la
        // anterior no puede desactivar la verificación para siempre, ni una que
        // estaba verde puede hacer que el primer fallo de hoy se atribuya al
        // agente. El permiso sí se conserva: el proyecto es el mismo.
        this.verifier.reset({ keepAuthorizations: true });
        this.ranCommands = false;
        this.context.addUser(this.task);
        this.bus.emit(EV.CHAT_USER, { id: uid('u'), text: this.task });

        // What previous runs on this project learned the hard way. Read once
        // per run and reused by explore and plan — the two phases where knowing
        // that something already failed changes what gets decided.
        await this.memory.load({ force: true });
        ...
        try {
            ...
            // `awaitingApproval` va en el evento porque en modo automático este
            // estado dura un tick: sin el dato, la interfaz anunciaba "esperando
            // tu aprobación" y la voz lo leía en alto mientras el agente ya
            // estaba escribiendo archivos.
            const policy = approvalPolicy(this.config);
            this._setState(STATE.AWAITING_APPROVAL);
            this.bus.emit(EV.PLAN_DRAFT, { plan: this.plan, awaitingApproval: !policy.plan });

            if (policy.plan) await this.approvePlan();
            return this.plan;
        } catch (err) {
    ...
    async approvePlan(editedPlan = null) {
        ...
        // Antes de tocar nada: ¿de qué color estaba la suite? Es lo único que
        // convierte "los tests fallan" en "los has roto tú", y también pide
        // permiso una sola vez si el comando salió del repositorio en vez de
        // de los ajustes. Si ya estaba en rojo, el verificador se desactiva
        // solo para esta tarea en lugar de culpar a cada paso.
        this.abort = this.abort && !this.abort.signal.aborted ? this.abort : new AbortController();
        // VERIFYING y no AWAITING_APPROVAL mientras corre: medir la suite puede
        // tardar minutos, y dejar el estado anterior mantenía vivo el botón
        // "Aprobar plan" — pulsarlo otra vez lanzaba una segunda ejecución.
        this._setState(STATE.VERIFYING);
        try {
            await this.verifier.captureBaseline({
                signal: this.abort.signal,
                requestApproval: (req) => this._requestApproval(req)
            });
        } catch (err) {
            if (isAbort(err)) return this._cancelled();
            this.logger.warn('No se pudo medir la línea base de los tests', { error: err.message });
        }

        // Cancelar durante la medida no puede dejar el motor atascado en un
        // estado ocupado del que `start()` ya no acepta otra tarea.
        if (this.abort.signal.aborted) return this._cancelled();

        this._setState(STATE.ACTING);

        if (approvalPolicy(this.config).steps) return await this.runAll();
        return this.plan;
    }
    ...
    async replanNow(reason = 'El usuario pidió replanificar.') {
        ...
        try {
            ...
            this._setState(STATE.AWAITING_APPROVAL);
            this.bus.emit(EV.PLAN_DRAFT, { plan: this.plan, awaitingApproval: true });
            return this.plan;
        } catch (err) {
            return this._fail(err);
        }
    ...
    async runAll() {
        ...
        if (this.paused && !this._cancelling) this._setState(STATE.PAUSED);
        return this.plan;
    }
    ...
    async cancel() {
        // `_cancelling` lo lee runAll() para no rematar en PAUSED: sin esto, el
        // bucle que seguía vivo llegaba a `if (this.paused) this._setState(PAUSED)`
        // DESPUÉS de que cancel() hubiera puesto IDLE, y la app se quedaba sin
        // poder aceptar una tarea nueva.
        this._cancelling = true;
        this.paused = true;
        this.abort?.abort();
        for (const p of this._approvals.values()) p.deferred.resolve(false);
    ...
    resolveApproval(id, approved) {
        const pending = this._approvals.get(id);
        if (!pending) return false;
        this._approvals.delete(id);

        // A command the user said no to is a preference, not an accident.
        // Remembered so the next run does not propose it again — being asked
        // the same question every day is how a tool starts to feel deaf.
        if (!approved && pending.req?.command) {
            (this.rejectedCommands ||= []).push(pending.req.command);
        }

        pending.deferred.resolve(!!approved);
        return true;
    }
    ...
    async _explore() {
        const tools = this.registry.forPhase('explore', {
            maxTools: this.profile.maxTools,
            allowWeb: this.config.get('tools.web', true)
        });
        ...
        for (let turn = 1; turn <= maxTurns; turn++) {
            ...
            const turnResult = await this._modelTurn({
                ...
                extraBlocks: turn === 1 ? [this.memory.block()] : []
            });
            instruction = null;
    ...
    async _plan() {
        ...
        for (let attempt = 0; attempt <= maxRepairs; attempt++) {
            ...
            const turn = await this._modelTurn({
                ...
                extraBlocks: attempt === 0 ? [this.memory.block()] : []
            });
    ...
    async _runStep(step) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (outcome.finished) {
                this._setState(STATE.VERIFYING);
                if (outcome.ranCommands) this.ranCommands = true;
                const verdict = await this._verifyStep(step, outcome.mutated, outcome.ranCommands);
                this._setState(STATE.ACTING);
    ...
    async _stepTurnLoop(step, attempt) {
        ...
        const allowWeb = this.config.get('tools.web', true);
        const tools = this.registry.forPhase('act', { maxTools: this.profile.maxTools, allowShell, allowWeb });
        ...
        // Un comando de consola que termina bien puede haber cambiado el
        // repositorio sin pasar por write_file: un codemod, un formateador, un
        // generador. No aparece en `mutated`, y aun así hay que verificar.
        ...
        for (let turn = 1; turn <= maxTurns; turn++) {
            ...
            if (this.paused && turn > 1) return { finished: false, paused: true, reason: 'pausado por el usuario', mutated: [...mutated], ranCommands };
            ...
            if (!parsed.calls.length) {
                if (parsed.unknown.length) {
                    ...
                    return { finished: false, reason: `el modelo dejó de llamar herramientas tras ${repairs} intentos de corrección`, mutated: [...mutated], ranCommands };
                } else {
                    instruction = nudgeInstruction(step);
                }
                if (repairs > this.config.get('agent.maxToolRepairs', 3)) {
                    return { finished: false, reason: 'el modelo no produjo llamadas válidas', mutated: [...mutated], ranCommands };
                }
                continue;
            }
            ...
            if (call.name === 'run_terminal_command' && result.ok) ranCommands = true;
            ...
            if (this.stepFinished !== null) {
                return { finished: true, summary: this.stepFinished, mutated: [...mutated], ranCommands };
            }
        }

        return { finished: false, reason: `se agotaron los ${maxTurns} turnos sin completar el paso`, mutated: [...mutated], ranCommands };
    }

    async _verifyStep(step, mutatedPaths, ranCommands = false) {
        ...
        // A step that read files and wrote nothing cannot have broken the
        // build, so there is nothing for the test suite to tell us. Skipping it
        // is what makes automatic verification affordable: on a five-step plan
        // where two steps are research, that is two whole test runs saved.
        // "Tocó algo" no es sólo write_file/edit_file. Un paso resuelto con
        // run_terminal_command — un codemod, un formateador, un generador —
        // deja `mutatedPaths` vacío y cambia el repositorio igual. Saltarse la
        // verificación ahí lo marcaría DONE sin comprobar nada, que es peor que
        // no tener verificación: es tenerla y que mienta.
        const project = (mutatedPaths.length || ranCommands)
            ? await this.verifier.projectCheck({
                signal: this.abort.signal,
                requestApproval: (req) => this._requestApproval(req)
            })
            : { ran: false, reason: 'el paso no leyó ni modificó nada del disco' };
        ...
        // Cancelada no es aprobada: decir "verificado" de un paso cuya
        // comprobación el usuario interrumpió es justo la mentira que el
        // verificador existe para no contar.
        if (!project.ran && /cancelada/i.test(project.reason || '')) {
            return { ok: false, issues: ['La verificación se canceló antes de terminar.'] };
        }
    ...
    async _finish({ aborted = false } = {}) {
        ...
        // El trabajo hecho por consola no aparece en `changes`, así que no basta
        // con mirar ahí; pero una tarea que no tocó NADA tampoco necesita el
        // veredicto final — y con la línea base ya medida al aprobar, eso serían
        // dos ejecuciones enteras de la suite para no cambiar ni un archivo.
        const project = (changes.length || this.ranCommands)
            ? await this.verifier.projectCheck({
                signal: this.abort?.signal,
                requestApproval: (req) => this._requestApproval(req)
            })
            : { ran: false, reason: 'la tarea no modificó nada' };
        ...
        // Written before DONE goes out, so anything reacting to the end of a
        // run already sees the note. Derived from the plan and the diffs, not
        // asked of the model: what failed and how many attempts it took are
        // facts we hold, and a weak model asked to write its own lessons writes
        // generic self-praise that costs context and says nothing.
     ...
     */
    async _modelTurn({ phase, system, instruction, tools = [], format = null, includeRepoMap = true, stream = true, escalate = 0, extraBlocks = [] }) {
    ...
    _requestApproval(req) {
        ...
        const id = uid('appr');
        const d = deferred();
        this._approvals.set(id, { deferred: d, req });
