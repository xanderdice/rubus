...
export class RepoMap {
    ...
    async build({ force = false, signal, focus = '' } = {}) {
        ...
        const treeText = renderTree(files, dirs);
        const header = [
    ...
    async detectConventions(root, files, signal) {
        ...
        const notes = [];
        /** Resolved into a command once the package manager is known. */
        ...
        if (pkgRaw) {
            try {
                ...
                if (pkg.scripts) {
                    // `npm init` writes a "test" script that does nothing but
                    // `echo "Error: no test specified" && exit 1`. Taking that
                    // as the project's test command would fail every single
                    // verification on a project that simply has no tests — and
                    // the failure would look like the agent broke something.
                    const testScript = String(pkg.scripts.test || '');
                    const placeholder = /no test specified/i.test(testScript)
                        || (/^\s*echo\b/i.test(testScript) && /exit\s+1\s*$/.test(testScript));
                    if (testScript && !placeholder) hasTestScript = true;
        ...
        // Resolved here, after the lock file has named the package manager:
        // running `npm test` in a pnpm workspace is how you get a wall of
        // missing-dependency errors that have nothing to do with the change.
        if (hasTestScript) {
            const pm = ['pnpm', 'yarn'].includes(data.packageManager) ? data.packageManager : 'npm';
            data.testCommand = `${pm} test`;
        }
...
function rankPath(rel, depth) {
    let score = 100 - depth * 12;
    if (ENTRY_HINTS.test(P.basename(rel))) score += 45;
    if (/\/(test|tests|spec|__tests__)\//i.test(`/${rel}`)) score -= 30;
    if (/\.(test|spec)\./i.test(rel)) score -= 30;
    if (/\.min\./i.test(rel)) score -= 200;
    if (/(^|\/)(src|lib|app|core|source)\//i.test(rel)) score += 20;
    return score;
}

/**
 * Every declaration in a file, as `{name, line, signature}`.
 *
 * The structured form is the real one; `extractSignatures` below is a string
 * rendering of it. Consumers that need the pieces — the outline tool, and the
 * search ranker deciding whether a hit is a definition or just a mention —
 * should not have to parse them back out of a formatted string.
 *
 * `limit` exists because the two uses want different things: a repo map wants
 * the first handful of symbols per file and nothing more, while search needs
 * to know about a definition wherever it happens to be in the file.
 */
export function extractSymbols(text, language, { limit = 40 } = {}) {
    ...
    for (let i = 0; i < lines.length && out.length < limit; i++) {
        ...
        for (const re of rules) {
            ...
            if (!m || !m[1]) continue;
            const signature = line.trim().replace(/\s*\{\s*$/, '').replace(/\s+/g, ' ').slice(0, 120);
            const key = `${m[1]}@${signature.length}`;
            if (seen.has(key)) break;
            seen.add(key);
            out.push({ name: m[1], line: i + 1, signature });
            break;
        }
    }
    return out;
...
/** The same thing rendered as `signature  ·line`, for the repo map. */
export function extractSignatures(text, language) {
    return extractSymbols(text, language).map(s => `${s.signature}  ·${s.line}`);
}

/**
 * How likely a file is to be the one you meant. Exported because search ranks
 * its hits the same way the map ranks its files — two different answers to
 * "which file matters here" would be a bug, not a feature.
 */
export function rankFile(f) {
    return rankPath(f.rel, f.depth);
}

/** Indented tree, capped, with big directories collapsed to a count. */
function renderTree(files, dirs) {
