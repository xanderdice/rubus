...
export const outlineFile = {
    ...
    async run(args, ctx) {
        ...
        // The line number leads: it is the part the model has to act on when it
        // comes back with read_file(around_line=…).
        const rows = symbols.map(s => `  L${String(s.line).padStart(5, ' ')}  ${s.signature}`);
