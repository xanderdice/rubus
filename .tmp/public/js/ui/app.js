...
import { $, $$, el, toast, wireDialogDismissal, openDialog, isAtBottom, shortTime } from './dom.js';
...
import { DesignPanel } from './design/design-panel.js';
...
class App {
    constructor({ platform, bus, engine }) {
        ...
        // El estudio no se construye hasta que se abre: importar el panel es
        // barato, pero montar PlayCanvas son 3,5 MB y un contexto WebGL que
        // quien sólo venga a programar no debería pagar nunca.
        this.design = new DesignPanel({ engine, bus });
        this.designOpen = false;
    }

    async start() {
        wireDialogDismissal();
        // Aplica el modo inicial ('plan') a la política de aprobación.
    ...
    /**
     * Alterna entre el chat y el estudio.
     *
     * Comparten el centro en vez de convivir: son dos trabajos distintos y
     * cada uno quiere toda la pantalla. Ir y volver conserva el estado de los
     * dos, así que no se pierde ni la conversación ni el lienzo.
     */
    wireDesign() {
        const chip = $('#chip-design');
        if (!chip) return;
        chip.onclick = async () => {
            this.designOpen = !this.designOpen;
            $('#chat-panel').hidden = this.designOpen;
            $('#design-panel').hidden = !this.designOpen;
            chip.classList.toggle('chip--on', this.designOpen);
            if (!this.designOpen) { this.design.hide(); return; }
            if (this.designOpen) {
                try {
                    await this.design.mount();
                } catch (err) {
                    toast('No se pudo abrir el estudio: ' + err.message);
                    this.engine.logger?.error('Fallo montando el estudio', { error: err.message });
                }
            }
        };
    }
    ...
    wireEvents() {
        ...
        on(EV.PLAN_DRAFT, ({ plan, awaitingApproval = true }) => {
            this.plan.setPlan(plan, { editable: true });
            this.selectTab('plan');
            this.chat.addSystem(
                `**Plan propuesto** — ${plan.steps.length} paso${plan.steps.length === 1 ? '' : 's'}. ` +
                (!awaitingApproval
                    ? 'Aprobado automáticamente; ya se está ejecutando. Pon la aprobación en «manual» en Ajustes para revisarlo antes.'
                    : this.mode === 'act'
    ...
    /**
     * Plan/Act es una barrera, no una etiqueta — y con la aprobación
     * automática dejó de serlo.
     *
     * `approve()` sí comprobaba el modo, pero `send()` no, y desde que
     * `approvalMode` viene en 'auto' el propio `engine.start()` aprueba y
     * ejecuta por dentro: mandar una tarea con el conmutador en «Plan (sólo
     * lectura)» escribía archivos en disco. El conmutador vuelve a mandar
     * fijando los dos ajustes finos, que ganan al modo; en Act se ponen a
     * `undefined` para que vuelva a decidir `approvalMode`.
     */
    setMode(mode) {
        this.mode = mode;
        const soloLectura = mode !== 'act';
        this.engine.config.set('agent.autoApprovePlan', soloLectura ? false : undefined);
        this.engine.config.set('agent.autoRunSteps', soloLectura ? false : undefined);
        for (const btn of $$('#mode-switch .mode')) {
