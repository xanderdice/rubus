...
export function wireSpeech(bus, speech, EV) {
    ...
    on(EV.PLAN_DRAFT, ({ plan, awaitingApproval = true }) => {
        ...
        // Sin mirar `awaitingApproval` la voz decía "pulsa aprobar" mientras el
        // agente ya estaba ejecutando: en modo automático ese estado dura un
        // tick y no hay ningún botón que pulsar.
        speech.say(
            `Plan propuesto: ${plan.goal}. ${plan.steps.length} pasos. ${steps}. ` +
            (awaitingApproval ? 'Pulsa aprobar para ejecutarlo.' : 'Empezando.')
        );
    });
