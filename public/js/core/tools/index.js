...
import { searchWeb, fetchUrl } from './web-tools.js';
...
const PHASE_TOOLS = {
    ...
    // cambio, es un read_file comprimido — se puede vivir sin él.
    act: ['read_file', 'edit_file', 'write_file', 'search_codebase', 'run_terminal_command', 'finish_step', 'search_web', 'fetch_url', 'outline_file', 'list_directory', 'think']
};

/** Herramientas que salen a la red. Se pueden apagar enteras. */
...
export class ToolRegistry {
     ...
     */
    forPhase(phase, { maxTools = 8, allowShell = true, allowWeb = true } = {}) {
        ...
        if (!allowWeb) names = names.filter(n => !WEB_TOOLS.has(n));
